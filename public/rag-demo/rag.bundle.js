...
var RAGKit = (() => {
  ...
  __export(browser_exports, {
    ...
    ExtractiveGenerator: () => ExtractiveGenerator,
  ...
  // src/chunker.ts
  var SENTENCE_BOUNDARY = /(?<=[.!?])\s+(?=[A-Z0-9"'(\[])/;
  function chunkText(text, options = {}) {
    ...
    for (const paragraph of paragraphs) {
      if (paragraph.length <= maxChars) {
        ...
        pieces.push(...paragraph.split(SENTENCE_BOUNDARY).map((s) => s.trim()).filter(Boolean));
      }
    }
    const chunks = [];
    let current = "";
    ...
    for (const piece of pieces) {
      ...
      flush();
      const overlap = overlapChars > 0 ? takeTail(current, overlapChars) : "";
      current = overlap ? overlap + " " + piece : piece;
      while (current.length > maxChars && piece.length > maxChars) {
  ...
  var LocalEmbedder = class {
    ...
    embedOne(text) {
      ...
      for (let i = 0; i < tokens2.length; i++) {
        bump(counts, tokens2[i]);
        if (i + 1 < tokens2.length) bump(counts, tokens2[i] + "_" + tokens2[i + 1]);
      }
      for (const [feature, count] of counts) {
        const weight = 1 + Math.log(count);
        const index = fnv1a(feature) % this.dimensions;
  ...
  function tokenize(text) {
    return text.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter((t) => t.length > 1 && !STOPWORDS.has(t));
  }
  ...
  // src/generators/extractive.ts
  var ExtractiveGenerator = class {
    constructor(options = {}) {
      this.maxSentences = options.maxSentences ?? 3;
    }
    async generate(question, context, options = {}) {
      const queryTerms = new Set(contentWords(question));
      if (context.length === 0 || queryTerms.size === 0) {
        return emit("I couldn't find anything relevant in the indexed documents.", options);
      }
      const candidates = [];
      for (const chunk of context) {
        for (const sentence of splitSentences(chunk.text)) {
          ...
          let overlap = 0;
          for (const w of words) if (queryTerms.has(w)) overlap++;
          const score = overlap / Math.sqrt(words.length) + chunk.score * 0.05;
          if (overlap > 0) candidates.push({ sentence: sentence.trim(), score });
        }
      }
      if (candidates.length === 0) {
        return emit("The retrieved context doesn't directly answer that question.", options);
      }
      ...
      for (const c of candidates) {
        ...
        seen.add(c.sentence);
        picked.push(c.sentence);
        if (picked.length >= this.maxSentences) break;
      }
      return emit(picked.join(" "), options);
    }
  };
  ...
  var MemoryVectorStore = class {
    ...
    async add(chunks, vectors) {
      if (chunks.length !== vectors.length) {
        throw new Error(`add(): got ${chunks.length} chunks but ${vectors.length} vectors`);
      }
      for (let i = 0; i < chunks.length; i++) {
        this.chunks.push(chunks[i]);
        this.vectors.push(vectors[i]);
        this.norms.push(l2(vectors[i]));
      }
    }
    async search(vector, k) {
      const queryNorm = l2(vector) || 1;
      const scored = this.chunks.map((chunk, i) => {
        let dot = 0;
        const v = this.vectors[i];
        for (let d = 0; d < v.length; d++) dot += v[d] * vector[d];
        return { ...chunk, score: dot / ((this.norms[i] || 1) * queryNorm) };
      });
  ...
  function tokens(text) {
    return text.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter((t) => t.length > 1 && !STOPWORDS2.has(t));
  }
  ...
  function overlapCount(aTokens, bTokens) {
    const b = new Set(bTokens);
    let hit = 0;
    for (const t of new Set(aTokens)) if (b.has(t)) hit++;
    return hit;
  }
  ...
  var BM25Index = class {
    ...
    add(chunks) {
      for (const chunk of chunks) {
        ...
        this.byId.set(chunk.id, { chunk, length: terms.length, tf });
    ...
    /** BM25 score for every indexed chunk against the query. Returns id → score. */
    scores(query) {
      const queryTerms = [...new Set(tokens(query))];
      const n = this.byId.size;
      const avgdl = n === 0 ? 0 : this.totalLength / n;
      const result = /* @__PURE__ */ new Map();
      if (n === 0 || avgdl === 0) return result;
      for (const [id, doc] of this.byId) {
        let score = 0;
  ...
  // src/evaluate.ts
  function evaluate(question, answer, sources) {
    const qT = uniqueTokens(question);
    const aT = uniqueTokens(answer);
    const ctxT = tokens(sources.map((s) => s.text).join(" "));
    const contextRelevance = sources.length ? Math.max(...sources.map((s) => coverage(qT, tokens(s.text)))) : 0;
    const groundedness = aT.length ? coverage(aT, ctxT) : 0;
    const answerRelevance = qT.length ? coverage(qT, aT) : 0;
    const cited = [];
    for (const s of sources) {
      if (coverage(aT, tokens(s.text)) >= 0.3) cited.push(s.id);
    }
    if (cited.length === 0 && aT.length) {
      let best = null;
      let bestOverlap = 0;
      for (const s of sources) {
        const o = overlapCount(aT, tokens(s.text));
        if (o > bestOverlap) {
          bestOverlap = o;
          best = s;
        }
      }
      if (best && bestOverlap >= 2) cited.push(best.id);
    }
    return {
  ...
  var RAG = class {
    constructor(options = {}) {
      ...
      this.embedder = options.embedder ?? new LocalEmbedder();
      this.store = options.store ?? new MemoryVectorStore();
      this.generator = options.generator ?? new ExtractiveGenerator();
      this.chunking = options.chunking ?? {};
      this.retrieval = { ...DEFAULT_RETRIEVAL, ...options.retrieval };
    }
    ...
    /**
     * Chunk, embed, and index one or more documents.
     * Accepts a plain string, a DocumentInput, or an array of either.
     * Returns the chunks that were indexed.
     */
    async add(input) {
      const docs = (Array.isArray(input) ? input : [input]).map(
        (d) => typeof d === "string" ? { text: d } : d
      );
      const allChunks = [];
      for (const doc of docs) {
        const docId = doc.id ?? `doc-${++this.docCounter}`;
        this.docIds.add(docId);
        const pieces = chunkText(doc.text, this.chunking);
      ...
      const vectors = await this.embedder.embed(allChunks.map((c) => c.text));
      await this.store.add(allChunks, vectors);
      this.bm25.add(allChunks);
      return allChunks;
    }
    /**
     * Retrieve the top-k chunks most relevant to a query. Hybrid by default:
     * pulls a broad candidate pool by vector similarity, scores the same chunks
     * with BM25, normalizes both signals, and reranks by their weighted blend.
     */
    async retrieve(query, k = 4, options = {}) {
      const opts = { ...this.retrieval, ...options };
      const [vector] = await this.embedder.embed([query]);
      if (!opts.hybrid || this.bm25.size === 0) {
        return this.store.search(vector, k);
      }
      const poolSize = Math.min(this.size, Math.max(k, k * opts.candidateMultiplier));
      const vecResults = await this.store.search(vector, poolSize);
      const lexScores = this.bm25.scores(query);
      const pool = /* @__PURE__ */ new Map();
      for (const r of vecResults) {
        pool.set(r.id, { chunk: stripScore(r), vec: r.score, lex: lexScores.get(r.id) ?? 0 });
      }
      const lexSorted = [...lexScores.entries()].sort((a, b) => b[1] - a[1]).slice(0, poolSize);
      for (const [id, score] of lexSorted) {
        if (pool.has(id)) continue;
        const chunk = this.bm25.get(id);
        if (chunk) pool.set(id, { chunk, vec: 0, lex: score });
      }
      const entries = [...pool.values()];
      const vecVals = entries.map((e) => e.vec);
      const lexVals = entries.map((e) => e.lex);
      const w = opts.keywordWeight;
      const scored = entries.map((e) => ({
        ...e.chunk,
        score: (1 - w) * normalize(e.vec, vecVals) + w * normalize(e.lex, lexVals)
      }));
      scored.sort((a, b) => b.score - a.score);
      return scored.slice(0, k);
    }
    /**
     * Retrieve relevant chunks, generate a grounded answer, and attach
     * lightweight quality metrics (context relevance, groundedness, answer
     * relevance) plus citations.
     */
    async query(question, options = {}) {
      const sources = await this.retrieve(question, options.topK ?? 4, options);
      const answer = await this.generator.generate(question, sources, { onToken: options.onToken });
      const metrics = evaluate(question, answer, sources);
  ...
  function stripScore(c) {
    return { id: c.id, docId: c.docId, index: c.index, text: c.text, metadata: c.metadata };
  }
...
Please migrate to a newer model. Visit https://docs.anthropic.com/en/docs/resources/model-deprecations for more information.`);
  ...
  var GROUNDING_SYSTEM = `You are a retrieval-augmented assistant. Answer the user's question using ONLY the information inside the provided <context> chunks.

Rules:
- Cite the chunks you used with bracketed numbers, e.g. [1] or [2][3].
- If the context does not contain the answer, say so plainly instead of guessing.
- Be concise and direct; answer first, brief supporting detail after.`;
  var ClaudeGenerator = class {
    constructor(options = {}) {
      ...
      this.maxTokens = options.maxTokens ?? 16e3;
      this.system = options.systemSuffix ? `${GROUNDING_SYSTEM}

${options.systemSuffix}` : GROUNDING_SYSTEM;
    }
    async generate(question, context, options = {}) {
      const contextBlock = context.map((c, i) => `<chunk index="${i + 1}" source="${escapeAttr(c.docId)}">
${c.text}
</chunk>`).join("\n");
...
Question: ${question}`;
      ...
      if (options.onToken) {
        stream.on("text", (delta) => options.onToken(delta));
      }
      const message = await stream.finalMessage();
      if (message.stop_reason === "refusal") {
